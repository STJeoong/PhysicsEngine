...
void World::reset()
{
	while (!s_gameObjects.empty())
		World::destroy(s_gameObjects[0]);
	s_root = nullptr;
}
...
void World::moveCollider(int key, const AABB& sweepAABB, const Vector2D& displacement) { if (key == -1) return; s_broadPhase.move(key, sweepAABB, displacement); }
